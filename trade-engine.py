...
class ProductionEngine:
    ...
    async def check_open_contracts(self, api):
        ...
        try:
            ...
            if contracts:
                ...
                logging.info(f"Recovered active contract: {self.current_contract}")
        except: pass

    def safe_handle_tick(self, msg, api):
        try: asyncio.create_task(self.handle_tick(msg, api))
        except: pass
    ...
    def save_stats(self):
        try:
            with open(self.stats_file, 'w') as f:
                json.dump({'shadow_wins': self.shadow_wins, 'shadow_losses': self.shadow_losses, 'session_profit': self.session_profit}, f)
        except: pass
    ...
    async def handle_tick(self, msg, api):
        ...
        if len(self.price_list) < 30: return
        ...
        # 3. AI (Lean DataFrame)
        # We only create a tiny DF for the single prediction row
        feat = pd.DataFrame([{
            'returns': (prices[-1] / prices[-2]) - 1,
            'volatility': np.std(np.diff(prices[-14:]) / prices[-15:-1]),
            'rsi': self.last_rsi,
        ...
        del feat # Force deletion to free RAM
    ...
    async def shadow_monitor(self, entry_price, side):
        ...
        win = (exit_p > entry_price) if side == "CALL" else (exit_p < entry_price)
    ...
    async def periodic_status_report(self):
        while True:
            ...
            try: await self.send_telegram_msg(await self.get_diagnostic_text())
            except: pass

    async def place_trade(self, api, direction):
        if self.current_contract: return
        params = {"buy": 1, "price": 1.0, "parameters": {"amount": 1.0, "basis": "stake", "contract_type": direction, "currency": "USD", "duration": 12, "duration_unit": "t", "symbol": "R_10"}}
        try:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if resp.status_code == 200:
                        for update in resp.json().get("result", []):
                            ...
                            if text == "/status": await self.send_telegram_msg(await self.get_diagnostic_text())
                            elif text == "/reset":
                                self.cooldown_until = 0
